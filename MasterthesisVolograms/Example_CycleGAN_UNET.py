...
NAME = "CycleGAN-UNET{}".format(int(time.time()))
...
with summary_writer.as_default():
  tf.summary.image("Training data example", plot_to_image(figure), step=0)

####################################################################################################
###### Building Blocks for Generator and discriminator
####################################################################################################
...
def upsample(
    ...
    x = tf.keras.layers.Concatenate(axis=3)([x, x_skip]) # (batch, heigth, width, channel)
...
def get_generator(
    ...
    # Upsampling
    for iter in range(num_upsample_blocks):
        filters //= 2
        x_skip = skips[iter]
        #print(x_skip.shape)
        x = upsample(x, x_skip, filters, activation=layers.Activation("relu"))
        #print(x.shape)
        #x = tf.keras.layers.Concatenate()([x, skips[iter]]) 
...
class GANMonitor(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        with summary_writer.as_default():
            tf.summary.image("Prediction", plot_to_image(figure), step= (epoch + 1))
